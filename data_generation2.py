...
for i in range (maxm): 
    ...
    for i in range(0, random_int):
        ...
        # current_row["id"] = counter
        # current_row["width"] = image_width
        # current_row["height"] = image_height
        # current_row["file_name"] = str(counter) + ".png"
        # current_row.append(str(counter) + ".png")
    ...
    # img.show()
        ...
        current_annotation = {}
        total_count+=1
    # folder_to_use = training_folder if counter <= (0.8 * maxm) else test_folder
    image = img.save(f"{save_directory}/"  + str(counter) + ".png")
    counter+=1

# writing to CSV
csv_filename = "text_box_data.json"
...
with open(csv_absolute_file_path, 'w+') as csvfile:
